...
public class ApplicationInitConfig {
    ...
    @NonFinal
    static final String ADMIN_PASSWORD = "khanh2104";
    ...
    ApplicationRunner applicationRunner(UserRepository userRepository, RoleRepository roleRepository) {
        return args -> {
            if (userRepository.findByPhone(ADMIN_USERNAME).isEmpty()) {
                roleRepository.save(Role.builder().roleName(PredefinedRole.USER_ROLE).roleDescription("User role").build());


                Role adminRole = roleRepository.save(Role.builder().roleName(PredefinedRole.ADMIN_ROLE).roleDescription("Admin role").build());

                var roles = new HashSet<Role>();

                roles.add(adminRole);

                User user = User.builder().phone(ADMIN_USERNAME).password(passwordEncoder.encode(ADMIN_PASSWORD)).build();
