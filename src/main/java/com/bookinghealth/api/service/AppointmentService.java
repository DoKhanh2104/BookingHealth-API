...
public class AppointmentService {
  ...
  public Page<AppointmentResponse> getAppointments(Pageable pageable) {
    return appointmentRepository
        ...
        .map(
            appointment -> {
              String patientName =
                  appointment.getUser() != null ? appointment.getUser().getName() : "";
              String patientPhone =
                  appointment.getUser() != null ? appointment.getUser().getPhone() : "";
              ...
              if (appointment.getDoctor() != null) {
                ...
                if (appointment.getDoctor().getSpecialties() != null) {
                  specialty =
                      appointment.getDoctor().getSpecialties().stream()
  ...
  @Transactional
  public com.bookinghealth.api.dto.response.client.AppointmentResponse bookAppointment(
    ...
    if (doctor.getPriceHistories() != null && !doctor.getPriceHistories().isEmpty()) {
      fee =
          doctor.getPriceHistories().stream()
    ...
    if (doctor.getUser() != null) {
      String timeSlotStr = "";
      if (saved.getAppointmentSlot() != null
          && saved.getAppointmentSlot().getTimeSlotTemplate() != null) {
      ...
      notificationService.createNotification(
          doctor.getUser(), "Lịch hẹn mới đang chờ duyệt", notifContent, 1);
    }
  ...
  public Page<com.bookinghealth.api.dto.response.client.AppointmentResponse> getMyAppointments(
    ...
    boolean isDoctor =
        user.getRoles() != null
            && user.getRoles().stream().anyMatch(role -> "DOCTOR".equals(role.getRoleName()));
  ...
  public void cancelAppointment(Long id) {
    ...
    if (isDoctor) {
      ...
      if (saved.getUser() != null) {
        String notifContent =
            String.format(
                "Bác sĩ %s đã hủy lịch hẹn của bạn vào lúc %s ngày %s.",
                currentUser.getName(), timeSlotStr, saved.getExpectedExaminationDate());
        notificationService.createNotification(
            saved.getUser(), "Lịch hẹn đã bị hủy", notifContent, 1);
      }
    } else {
      ...
      if (saved.getDoctor() != null && saved.getDoctor().getUser() != null) {
        String notifContent =
            String.format(
                "Bệnh nhân %s đã hủy lịch hẹn vào lúc %s ngày %s.",
                currentUser.getName(), timeSlotStr, saved.getExpectedExaminationDate());
        notificationService.createNotification(
            saved.getDoctor().getUser(), "Lịch hẹn đã bị hủy bởi bệnh nhân", notifContent, 1);
      }
    }
  }
  ...
  public void confirmAppointment(Long id) {
    ...
    if (saved.getUser() != null) {
      ...
      String notifContent =
          String.format(
              "Bác sĩ %s đã xác nhận lịch hẹn khám của bạn vào lúc %s ngày %s.",
              currentUser.getName(), timeSlotStr, saved.getExpectedExaminationDate());
      notificationService.createNotification(
          saved.getUser(), "Lịch hẹn đã được xác nhận", notifContent, 1);
    }
  }
  ...
  public void completeAppointment(Long id, CompleteAppointmentRequest request) {
    ...
    if (saved.getUser() != null) {
      ...
      String notifContent =
          String.format(
              "Kết quả ca khám của bạn vào lúc %s ngày %s đã có. Vui lòng xem bệnh án và đơn thuốc chi tiết.",
              timeSlotStr, saved.getExpectedExaminationDate());
      notificationService.createNotification(
          saved.getUser(), "Kết quả khám bệnh đã sẵn sàng", notifContent, 1);
    }
  }
  ...
  public DoctorDashboardResponse getDoctorDashboardStats() {
    ...
    List<Appointment> todayAppointments =
        appointmentRepository.findByDoctorIdAndExpectedExaminationDate(doctor.getId(), today);

    int todayTotal = todayAppointments.size();
    int todayPending = (int) todayAppointments.stream().filter(app -> app.getStatus() == 0).count();
    int todayCompleted =
        (int) todayAppointments.stream().filter(app -> app.getStatus() == 2).count();

    int uniquePatients =
        (int)
            allDoctorAppointments.stream()
    ...
    double monthlyRevenue =
        allDoctorAppointments.stream()
            ...
            .filter(
                app ->
                    app.getExpectedExaminationDate() != null
    ...
    List<com.bookinghealth.api.dto.response.client.AppointmentResponse> featured =
  ...
  public Page<com.bookinghealth.api.dto.response.client.AppointmentResponse>
      getPatientAppointmentsForAdmin(Long patientId, Pageable pageable) {
