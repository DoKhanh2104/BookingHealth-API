...
public class DashboardService {
    ...
    public DashboardSummaryResponse getSummary(String period) {
        ...
        double totalRevenue = appointmentRepository.sumRevenueByDateRange(from, to);
    ...
    public List<SpecialtyDistributionItem> getSpecialtyDistribution(String period) {
        ...
        return rows.stream()
                .map(row -> SpecialtyDistributionItem.builder()
    ...
    public List<TopDoctorResponse> getTopDoctors(String period) {
        // Lấy top 5
        List<Object[]> rows = appointmentRepository.topDoctorsByBookingsAllTime(
                PageRequest.of(0, 5));
        ...
        for (Object[] row : rows) {
            ...
            doctorRepository.findById(doctorId).ifPresent(doc -> {
                ...
                String specName   = doc.getSpecialties() != null && !doc.getSpecialties().isEmpty()
                        ? doc.getSpecialties().iterator().next().getSpecialtyName()
                        : "";

                result.add(TopDoctorResponse.builder()
                        .name(doctorName)
                        .spec(specName)
    ...
    public List<RecentFeedbackResponse> getRecentFeedbacks() {
        List<DoctorReview> reviews = doctorReviewRepository
                .findRecentLowRatingFeedbacks(2, PageRequest.of(0, 5));
    ...
    public List<PendingDoctorResponse> getPendingDoctors() {
        ...
        return pendingDoctors.stream().map(doc -> {
            String name = doc.getUser() != null ? doc.getUser().getName() : "Bác sĩ";
            String spec = doc.getSpecialties() != null && !doc.getSpecialties().isEmpty()
                    ? doc.getSpecialties().iterator().next().getSpecialtyName()
                    : "";
            String date = doc.getPracticeStartDate() != null
                    ? doc.getPracticeStartDate().format(fmt)
                    : "";
            String certStatus = doc.getPracticeLicenseImage() != null && !doc.getPracticeLicenseImage().isBlank()
                    ? "Đã tải lên"
                    : "Chưa tải lên";
    ...
    private LocalDate[] getDateRange(String period) {
        ...
        return switch (period) {
            ...
            case "month" -> new LocalDate[]{today.with(TemporalAdjusters.firstDayOfMonth()), today.with(TemporalAdjusters.lastDayOfMonth())};
