...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
  ...
  @Query(
      "SELECT COUNT(a) FROM Appointment a WHERE a.expectedExaminationDate BETWEEN :from AND :to AND a.status = :status")
  long countByDateRangeAndStatus(
      @Param("from") LocalDate from, @Param("to") LocalDate to, @Param("status") int status);

  long countByStatus(int status);

  /** Tổng doanh thu trong khoảng ngày (chỉ tính booking đã hoàn thành - status=2) */
  @Query(
      "SELECT COALESCE(SUM(a.totalAmount), 0) FROM Appointment a WHERE a.expectedExaminationDate BETWEEN :from AND :to AND a.status = 2")
  double sumRevenueByDateRange(@Param("from") LocalDate from, @Param("to") LocalDate to);

  /** Tổng doanh thu toàn bộ thời gian */
  @Query("SELECT COALESCE(SUM(a.totalAmount), 0) FROM Appointment a WHERE a.status = 2")
  double sumRevenueAllTime();

  /** Doanh thu theo từng ngày trong khoảng (dùng cho revenue chart - weekly) */
  @Query(
      "SELECT a.expectedExaminationDate, COALESCE(SUM(a.totalAmount), 0) FROM Appointment a WHERE a.expectedExaminationDate BETWEEN :from AND :to AND a.status = 2 GROUP BY a.expectedExaminationDate ORDER BY a.expectedExaminationDate")
  List<Object[]> revenueGroupByDay(@Param("from") LocalDate from, @Param("to") LocalDate to);

  /** Doanh thu theo năm-tháng (YYYY-MM) */
  @Query(
      value =
          "SELECT DATE_FORMAT(a.ngayKhamDuKien, '%Y-%m') as ym, COALESCE(SUM(a.tongTien), 0) FROM LICH_HEN a WHERE a.ngayKhamDuKien BETWEEN :from AND :to AND a.trangThai = 2 GROUP BY ym ORDER BY ym",
      nativeQuery = true)
  ...
  @Query(
      "SELECT a.doctor.id, COUNT(a) FROM Appointment a WHERE a.expectedExaminationDate BETWEEN :from AND :to AND a.status = 2 GROUP BY a.doctor.id ORDER BY COUNT(a) DESC")
  List<Object[]> topDoctorsByBookings(
      @Param("from") LocalDate from, @Param("to") LocalDate to, Pageable pageable);

  @Query(
      "SELECT a.doctor.id, COUNT(a) FROM Appointment a WHERE a.status = 2 GROUP BY a.doctor.id ORDER BY COUNT(a) DESC")
  List<Object[]> topDoctorsByBookingsAllTime(Pageable pageable);

  /** Phân bố booking theo chuyên khoa trong khoảng ngày */
  @Query(
      "SELECT s.specialtyName, COUNT(a) FROM Appointment a JOIN a.doctor d JOIN d.specialties s WHERE a.expectedExaminationDate BETWEEN :from AND :to GROUP BY s.specialtyName ORDER BY COUNT(a) DESC")
  List<Object[]> bookingsBySpecialty(@Param("from") LocalDate from, @Param("to") LocalDate to);

  @Query(
      "SELECT s.specialtyName, COUNT(a) FROM Appointment a JOIN a.doctor d JOIN d.specialties s GROUP BY s.specialtyName ORDER BY COUNT(a) DESC")
  ...
  @Query(
      "SELECT d.id AS doctorId, "
          ...
          + "FROM Appointment a "
          + "JOIN a.doctor d "
          + "WHERE (cast(:fromDate as date) IS NULL OR a.expectedExaminationDate >= :fromDate) "
          + "AND (cast(:toDate as date) IS NULL OR a.expectedExaminationDate <= :toDate) "
