...
public interface DoctorReviewRepository extends JpaRepository<DoctorReview, Long> {
  ...
  /**
   * Lấy feedback có rating thấp (≤ maxRating) sắp xếp theo ID giảm dần (mới nhất trước).
   * Kèm thông tin bác sĩ và bệnh nhân.
   */
  @Query("SELECT r FROM DoctorReview r JOIN FETCH r.doctor d JOIN FETCH d.user du JOIN FETCH r.user u WHERE r.rating <= :maxRating ORDER BY r.id DESC")
  List<DoctorReview> findRecentLowRatingFeedbacks(@Param("maxRating") int maxRating, Pageable pageable);
  ...
  @Query("SELECT r.doctor.id AS doctorId, " +
         ...
         "AND (:clinicId IS NULL OR r.doctor.clinic.id = :clinicId) " +
         "AND (cast(:fromDate as date) IS NULL OR r.appointment.expectedExaminationDate >= :fromDate) " +
